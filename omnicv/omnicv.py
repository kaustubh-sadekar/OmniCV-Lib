...
class fisheyeImgConv:
    ...
    def fisheye2equirect(
        self, srcFrame, outShape, aperture=0, delx=0, dely=0, radius=0, edit_mode=False
    ):
    ...
    def equirect2Fisheye(self, img, outShape, f=50, xi=1.2, angles=[0, 0, 0]):
        ...
        self.fmin = (
            np.sqrt(
                -(1 - self.xi ** 2) * ((xref - self.Cx) ** 2 + (yref - self.Cy) ** 2)
            )
            * 1.0001
        )
    ...
    def equirect2Fisheye_EUCM(
        self, img, outShape, f=50, a_=0.5, b_=0.5, angles=[0, 0, 0]
    ):
        self.Hd = outShape[0]
        self.Wd = outShape[1]
        self.f = f
        ...
        mz = np.real(
            ...
            / (
                self.a_ * np.lib.scimath.sqrt(1 - (2 * self.a_ - 1) * self.b_ * r_2)
                + (1 - self.a_)
            )
        )
    ...
    def equirect2Fisheye_DS(
        self, img, outShape, f=50, a_=0.5, xi_=0.5, angles=[0, 0, 0]
    ):
        self.Hd = outShape[0]
        self.Wd = outShape[1]
        self.f = f
        ...
        # self.fmin = np.sqrt(-(1-self.xi**2)*((xref-self.Cx)**2 + (yref-self.Cy)**2))*1.0001
        self.fmin = np.sqrt(
            np.abs((2 * self.a_ - 1) * ((xref - self.Cx) ** 2 + (yref - self.Cy) ** 2))
        )
        ...
        mz = np.real(
            ...
            / (self.a_ * np.lib.scimath.sqrt(1 - (2 * self.a_ - 1) * r_2) + 1 - self.a_)
        )

        omega = np.real(
            (mz * self.xi_ + np.lib.scimath.sqrt(mz ** 2 + (1 - self.xi_ ** 2) * r_2))
            / (mz ** 2 + r_2)
        )
            ...
            if self.dice:
                line1 = np.hstack(
                    (
                        ...
                        cv2.flip(dstFrame[:, 4 * self.side : 5 * self.side, :], 0),
                        dstFrame[:, 4 * self.side : 5 * self.side, :] * 0,
                        dstFrame[:, 4 * self.side : 5 * self.side, :] * 0,
                    )
                )
                line2 = np.hstack(
                    (
                        ...
                        cv2.flip(dstFrame[:, 1 * self.side : 2 * self.side, :], 1),
                        cv2.flip(dstFrame[:, 2 * self.side : 3 * self.side, :], 1),
                    )
                )
                line3 = np.hstack(
                    (
