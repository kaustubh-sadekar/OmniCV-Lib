...
void fisheyeImgConv::eqrect2cubemap(const cv::Mat &srcFrame, cv::Mat &dstFrame, const int side, const bool modif, const bool dice)
{
	this->dice = dice;
	this->side = side;
...
void fisheyeImgConv::cubemap2equirect(const cv::Mat &srcFrame1, const cv::Size outShape, cv::Mat &dstFrame)
{
	...
	{
		cv::Mat t;
		cv::flip(srcFrame1.rowRange(0,side).colRange(side*4,side*5), t ,0);
		t.copyTo(srcFrame1.rowRange(0,side).colRange(side*4,side*5));
