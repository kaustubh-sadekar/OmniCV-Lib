...
print("\n\n########################################################\n")
print("Running visual tests for python library ....")

mapper = fisheyeImgConv()

cv2.namedWindow("output image and reference image", cv2.WINDOW_NORMAL)
cv2.resizeWindow("output image and reference image", 800, 400)

img_path = "./input/equirect.jpg"
assert os.path.isfile(
    img_path
), "Wrong path... Run the tests.py code after changing the directory inside test folder"
...
cv2.imshow(
    "output image and reference image", np.hstack((fisheye, cv2.imread(img_path)))
)
cv2.waitKey(0)
assert os.path.isfile(img_path), "Output reference image not present"
assert fisheye.shape[:2] == (
    250,
    250,
), "Output dimensions for generated fisheye image do not match !"

print("[2] Testing equirect2Fisheye_EUCM method ...")
fisheye = mapper.equirect2Fisheye_EUCM(
    equiRect, outShape=[250, 250], f=100, a_=0.4, b_=2, angles=[0, 0, 0]
)
...
fisheye = mapper.equirect2Fisheye_FOV(
    equiRect, outShape=[250, 250], f=40, w_=0.5, angles=[0, 0, 0]
)
...
fisheye = mapper.equirect2Fisheye_DS(
    equiRect, outShape=[250, 250], f=90, a_=0.4, xi_=0.8, angles=[0, 0, 0]
)
...
cv2.imshow(
    "output image and reference image", np.hstack((equiRect, cv2.imread(img_path)))
)
cv2.waitKey(0)
assert os.path.isfile(img_path), "Output reference image not present"
assert equiRect.shape[:2] == (
    200,
    400,
), "Output dimensions for generated fisheye image do not match !"
...
cv2.imshow(
    "output image and reference image", np.hstack((cubemap, cv2.imread(img_path)))
)
cv2.waitKey(0)
assert os.path.isfile(img_path), "Output reference image not present"
assert cubemap.shape[:2] == (
    256,
    1536,
), "Output dimensions for generated fisheye image do not match !"
...
cv2.imshow("output image and reference image", np.hstack((persp, cv2.imread(img_path))))
cv2.waitKey(0)
assert os.path.isfile(img_path), "Output reference image not present"
assert persp.shape[:2] == (
    400,
    400,
), "Output dimensions for generated fisheye image do not match !"
