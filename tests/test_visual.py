...
print("\n\n")
input("Visual tests need user input for verification ...")

cv2.namedWindow("output image and reference image",0)
...
def test_equirect2Fisheye_UCM():
    ...
    cv2.imshow("output image and reference image",np.hstack((fisheye,cv2.imread(img_path))));
    cv2.waitKey(0)
    ok = input("enter y if output and reference image match")
    assert ok.lower() == 'y', "Visual test failure reported by user for equirect2Fisheye_UCM method"
...
cv2.imshow("output image and reference image",np.hstack((equiRect,cv2.imread(img_path))));
cv2.waitKey(0)
assert os.path.isfile(img_path),"Output reference image not present"
...
cv2.imshow("output image and reference image",np.hstack((cubemap,cv2.imread(img_path))));
cv2.waitKey(0)
assert os.path.isfile(img_path),"Output reference image not present"
